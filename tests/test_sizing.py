...
def test_strong_long_full_size():
    ...
    assert abs(size.borrow - 4_000.0) < 1e-9  # seed_usd * (3 - 1) USDC
...
def test_moderate_long_half_size():
    ...
    assert abs(size.borrow - 2_000.0) < 1e-9


def test_long_leverage_2x():
    size = compute(10_000.0, 1000.0, _sig(1.0, "long"), _cfg(leverage=2.0))
    assert abs(size.borrow - 2_000.0) < 1e-9  # seed_usd * (2-1) USDC
...
def test_strong_short_full_size():
    ...
    # MCP flash-loan loop: supply=lev×seed USDC, borrow=(lev-1)×seed asset
    ...
    assert abs(size.borrow - 1.0) < 1e-9  # (lev-1)*seed/price = 1 ETH


def test_moderate_short_half_size():
    # Shorts open at full size because prepare_increase targets leverage, not seed.
    size = compute(10_000.0, 2000.0, _sig(0.5, "short"), _cfg())
    assert size.seed_usd == 2_000.0
    assert abs(size.supply - 2_000.0) < 1e-9
...
def test_short_capped_at_short_max_leverage():
    ...
    size = compute(
        10_000.0, 2000.0, _sig(1.0, "short"), _cfg(leverage=3.0, short_max_leverage=2.0)
    )
    assert size.seed_usd == 2_000.0
    assert abs(size.supply - 2_000.0) < 1e-9  # USDC collateral
    # borrow = seed * (lev-1) / price = 2000*1/2000 = 1.0 ETH
    assert abs(size.borrow - 1.0) < 1e-9
...
def test_short_below_cap_uses_configured_leverage():
    ...
    size = compute(
        10_000.0, 2000.0, _sig(1.0, "short"), _cfg(leverage=1.5, short_max_leverage=2.0)
    )
    assert size.seed_usd == 2_000.0
    # borrow = seed * (lev-1) / price = 2000 * 0.5 / 2000 = 0.5 ETH
