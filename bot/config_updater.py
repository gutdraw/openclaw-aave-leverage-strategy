...
BOUNDS: dict[str, tuple[float, float]] = {
    ...
    "signal_reversal_min_score": (
        0,
        2,
    ),  # int: 0=only strong flip, 1=moderate+strong, 2=any flip
...
def propose(
    ...
    if not raw.get("paper_trading", True):
        return UpdateResult(
            ...
            applied={},
            rejected={
                k: "live mode — switch to paper_trading: true before changing parameters"
                for k in changes
            },
    ...
    for field, new_val in changes.items():
        ...
        try:
            if field in BOUNDS:
                ...
                if isinstance(new_val, bool):
                    pass
                elif isinstance(new_val, str) and new_val.strip().lower() in {
                    "true",
                    "false",
                }:
                    new_val = new_val.strip().lower() == "true"
                else:
        ...
        if field in BOUNDS:
            ...
            if not (lo <= new_val <= hi):
                rejected[field] = f"value {new_val} out of bounds [{lo}, {hi}]"
                continue

        # ── Cross-field sanity: hf_defense_close < hf_defense_reduce ─────
        if field == "hf_defense_close":
            reduce_val = changes.get(
                "hf_defense_reduce", raw.get("hf_defense_reduce", 1.35)
            )
        ...
        if field == "hf_defense_reduce":
            close_val = changes.get(
                "hf_defense_close", raw.get("hf_defense_close", 1.20)
            )
        ...
        if field == "short_hf_defense_close":
            reduce_val = changes.get(
                "short_hf_defense_reduce", raw.get("short_hf_defense_reduce", 1.09)
            )
            if new_val >= float(reduce_val):
                rejected[field] = (
        ...
        if field == "short_hf_defense_reduce":
            close_val = changes.get(
                "short_hf_defense_close", raw.get("short_hf_defense_close", 1.05)
            )
            if new_val <= float(close_val):
                rejected[field] = (
...
def get_change_history(changes_log: str = "config_changes.jsonl") -> list[dict]:
    ...
    return [
        e for e in state.load_entries(changes_log) if e.get("type") == "config_change"
    ]
